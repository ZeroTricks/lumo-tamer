...
export class ConversationStore {
    ...
    /**
     * Record a call_id with its tool name for this conversation
     */
    addGeneratedCallId(id: ConversationId, callId: string, toolName: string): void {
        const state = this.getOrCreate(id);
        state.generatedCallIds.set(callId, toolName);
    }

    /**
     * Get the tool name for a call_id generated by this conversation
     */
    getToolNameForCallId(id: ConversationId, callId: string): string | undefined {
        const state = this.conversations.get(id);
        return state?.generatedCallIds.get(callId);
    }
    ...
    private createEmptyState(id: ConversationId): ConversationState {
        ...
        return {
            ...
            generatedCallIds: new Map<string, string>(),  // call_id -> tool_name mapping
        };
    }
